...
class GaeSampleMemory(object):
    ...
    class Path(object):
        ...
        def merge(self, key):
            ...
            #results_tensor = torch.Tensor.cpu(results_tensor)
    ...
    class Trajectory(object):
        ...
        def merge(self, key):
            ...
            #merged_results_tensor = torch.tensor(merged_results)
            #merged_results_tensor = torch.Tensor.cpu(merged_results_tensor)
    ...
    def _compute_gae(self, rewards, values, dones, discount_gamma, discount_lambda):
        ...
        #values[:len(adv)] = torch.Tensor.cpu(values[:len(adv)])
        #values_tensor = torch.tensor(values[:len(adv)])
        #values_tensor = torch.Tensor.cpu(values_tensor)
