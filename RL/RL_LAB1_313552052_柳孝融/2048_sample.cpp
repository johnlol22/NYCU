...
public:
	 ...
	 * set a 16-bit row
	 */
	void place(int i, int r) { raw = (raw & ~(0xffffULL << (i << 4))) | (uint64_t(r & 0xffff) << (i << 4)); } //ULL means unsigned long long
	/**
	 * get a 4-bit tile
	 */
...
private:
	...
	struct lookup {
		...
		void move_left(uint64_t& raw, int& sc, int i) const { //left may be 0x0000 to 0xffff
			raw |= uint64_t(left) << (i << 4);				  //i = 0,1,2,3
			sc += score;									  //score is the points of 0x0000 to 0xffff 
		}
		...
		lookup() {		//struct constructor
			static int row = 0;	//row = 0~65535
			init(row++);		//there will be 65536 lefts and rights. scores will contain from 0x0000 to 0xffff
		}

		static const lookup& find(int row) {
			static const lookup cache[65536];	//65536 lookup structs (0~65535) will be established when find 
			return cache[row];					//is called.
		}
	};
...
public:
	...
	int move_left() {	//scores that after moving left, if it's the same, meaning that you can not move left
		uint64_t move = 0;
		uint64_t prev = raw;
		int score = 0;
		lookup::find(fetch(0)).move_left(move, score, 0);//row 0,1,2,3 are assemble to produce move
		lookup::find(fetch(1)).move_left(move, score, 1);//score is the score of this board
...
public:
	...
	virtual float estimate(const board& b) const {
		...
		float res = 0;
		for (int i=0;i<8;i++){
			size_t j = indexof(isomorphic[i], b);
			res = res + operator[](j);
		}
		return res;
	}
	...
	virtual float update(const board& b, float u) {
		...
		for (int i=0;i<8;i++){
			size_t j = indexof(isomorphic[i], b);
			operator[](j) = operator[](j) + u / isomorphic[i].size();
			res += operator[](j);
		}
		return res;
	}
...
class state {
public:
	state(int opcode = -1)	//esti initializes to maximum floating point
...
public:
	...
	bool assign(const board& b) {
		...
		after = before = b;
		score = after.move(opcode);//return score after moving up or down or left or right
		esti = score;
		return score != -1;
	}
...
public:
	 ...
	 */
	float estimate(const board& b) const {	//4 features 8 isomorphic (32 in total) to lookup the weight table
		debug << "estimate " << std::endl << b;	//the summation is the score of this board
		float value = 0;
	...
	float update(const board& b, float u) const {
		...
		float u_split = u / feats.size();	//divide by 4
		float value = 0;
		for (feature* feat : feats) {	//u_split divide into 8 isomorphic
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				// TODO
				//move is a state class which contains before state, after state, esti, score
				//after assign function call complete, we have already update before state, after state
				//and score
				//notes: difference between reward() and value(): reward() return the reward after the state
				//move whether up, right, down or left;value() return the estimate value of the entire board
				

				//we have new reward, after state, so we update the state's value by the reward of this movement,
				// that is, up right down or left, plus the estimate value of the board(after state)
				//select the maximum value, then we have the best action

				float total = 0;
				//don't need to consider the probability of which empty space popup 2 or 4
				//only consider the value of the board after the new num popup
				board after_state = move->after_state();
				for (int i=0;i<16;i++){
					if (after_state.at(i) == 0){
						board tmp_board = after_state;
						tmp_board.set(i, 1);
						total += 0.9 * estimate(tmp_board);
						tmp_board = after_state;
						tmp_board.set(i, 2);
						total += 0.1 * estimate(tmp_board);
					}
				}
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		// TODO
		//target means the next target score of the current board 
		float target = 0;	//the learning target of each state, path[path.size()-1] is final state,
		float error;		//so its learning target will be 0
		for (int i=path.size()-2;i>=0;i--){
			//error = target - estimate(path[i].after_state());
			//target = path[i].reward() + update(path[i].after_state(), alpha * error);

			error = (path[i].reward() + target) - estimate(path[i].before_state());
			float state_newVal = update(path[i].before_state(), alpha * error);
			target = state_newVal;
...
int main(int argc, const char* argv[]) {
	...
	size_t total = 100000;
	unsigned seed;
	__asm __volatile__ ("rdtsc" : "=a" (seed)); //__asm__
