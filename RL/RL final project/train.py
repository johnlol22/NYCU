...
eval_callback = EvalCallback(env, 
                           best_model_save_path='./td3_13_model/td3_2',
                           log_path='./td3_racecar_tensorboard/TD3_13_eval', 
                           eval_freq=500,
                           deterministic=True, 
                           render=False)
...
action_noise = OrnsteinUhlenbeckActionNoise(mean=np.zeros(n_actions), sigma=np.array([0.2, 0.1]), theta=0.15)
...
model = TD3(
    ...
    gamma=0.98,
    max_grad_norm=0.5,
    tensorboard_log="./td3_racecar_tensorboard/TD3_14/"
)
model.learn(total_timesteps=1e8, log_interval=10, callback=eval_callback)
model.save("./td3_14.zip")
