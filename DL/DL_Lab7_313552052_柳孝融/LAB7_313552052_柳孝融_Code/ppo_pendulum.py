...
# Add this class for observation normalization
class RunningMeanStd:
    """Tracks the mean and standard deviation of a data stream."""
    ...
    def normalize(self, x):
        """Normalize data using the current mean and variance."""
...
class Actor(nn.Module):
    ...
    def forward(self, state: torch.Tensor) -> torch.Tensor:
        """Forward method implementation with batch support."""
        ...
        # Get mean and constrained log standard deviation
        mean = self.mean_layer(x)
        log_std = torch.clamp(self.log_std_layer, self.log_std_min, self.log_std_max)
...
class Critic(nn.Module):
    def __init__(self, in_dim: int):
        ...
        ############TODO#############
        # Remeber to initialize the layer weights
        # Critic network architecture - 2 hidden layers
        self.hidden1 = nn.Linear(in_dim, 64)
        self.hidden2 = nn.Linear(64, 64)
        ...
        init_layer_uniform(self.hidden1)
        init_layer_uniform(self.hidden2)
        init_layer_uniform(self.output)
        #############################
...
class PPOAgent:
    ...
    def __init__(self, env: gym.Env, args):
        ...
        # Reward normalization flag (can be turned on/off)
        self.normalize_rewards = True
        self.normalize_observations = True
        
        # Value for reward scaling (helps with different reward scales)
    ...
    def normalize_observation(self, obs):
        ...
        if self.normalize_observations:
            if not self.is_test:  # Only update statistics during training
    ...
    def normalize_reward(self, reward):
        ...
        if self.normalize_rewards:
            ...
            return np.clip(reward / np.sqrt(self.reward_rms.var + 1e-8), -10.0, 10.0) * self.reward_scaling
        return reward
    ...
    def select_action(self, state: np.ndarray) -> np.ndarray:
        ...
        state = torch.FloatTensor(normalized_state).to(self.device)
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, np.float64, bool]:
        ...
        normalized_reward = self.normalize_reward(reward) if not self.is_test else reward
        if not self.is_test:
            self.rewards.append(torch.FloatTensor(normalized_reward).to(self.device))
    ...
    def update_model(self, next_state: np.ndarray) -> Tuple[float, float]:
        ...
        for state, action, old_value, old_log_prob, return_, adv in ppo_iter(
            ...
            # We use negative because we're minimizing the loss (and want to maximize the objective)
    ...
    def train(self):
        """Train the PPO agent with batch processing during updates."""
        # self.is_test = False
        ...
        for ep in tqdm(range(1, self.num_episodes)):
            score = 0
            print("\n")
            for _ in range(self.rollout_len):  # Collect one trajectory
                self.total_step += 1
    ...
    def test(self, video_folder: str):
        """Test the agent."""
        # self.is_test = True
        seed_list = [77, 78, 80, 81, 83, 84, 86, 87, 88, 89, 90, 91, 92, 94, 95, 96, 97, 99, 100, 102]
        tmp_env = self.env
        total_score = 0
        for i in seed_list:
            random.seed(i)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wandb-run-name", type=str, default="pendulum-ppo-run11")
    ...
    parser.add_argument("--checkpoint-path", type=str, default="./result/ppo11")
    ...
    if not args.test:
        ...
        agent.test("./ppo_pendulum_test_video")