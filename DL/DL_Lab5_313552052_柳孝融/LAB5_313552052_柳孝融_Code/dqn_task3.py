...
class DQNAgent:
    def __init__(self, env_name="ALE/Pong-v5", args=None):
        ...
        self.memory = MultiStepPrioritizedReplayBuffer(
            capacity=args.memory_size,
            n_steps=5,  # Typical value, you can experiment
            gamma=self.gamma,
            alpha=0.6,  # For PER
            beta=0.4    # For PER
        )
    ...
    def run(self, episodes=500):
        for ep in range(episodes):
            ...
            if len(self.memory) >= self.replay_start_size and self.epsilon > self.epsilon_min:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-dir", type=str, default="./results/task3_2")
    parser.add_argument("--wandb-run-name", type=str, default="pong-run_task3_2")
    ...
    parser.add_argument("--train-per-step", type=int, default=7)
    args = parser.parse_args()

    wandb.init(project="DLP-Lab5-DQN-Pong_task3_2", name=args.wandb_run_name, save_code=True)
