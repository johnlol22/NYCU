...
#mask_b: iteration decoding initial mask, where mask_b is true means mask
    def inpainting(self,image,mask_b,i): #MakGIT inference
        ...
        self.model.eval()
        with torch.no_grad():
            # Get latent representation and z_q
            latent, z_q = self.model.encode_to_z(image)

            # Get indices from the codebook
            _, z_indices, _ = self.model.vqgan.codebook(self.model.vqgan.quant_conv(latent))
            z_indices = z_indices.reshape(image.shape[0], -1)

            # Apply mask
            z_indices[mask_b] = self.model.mask_token_id

            # Initial values for the decoding loop
            z_indices_predict = z_indices.clone()
            mask_bc = mask_b.clone()
        
            # Iterative decoding loop
            for step in range(self.total_iter):
                if step == self.sweet_spot and self.sweet_spot > 0:
                    break
                ratio = step / max(1, (self.total_iter - 1))        # controls how many tokens unmasked

                # No need to pass ratio - model.inpainting handles this internally
                z_indices_new, mask_new = self.model.inpainting(z_indices_predict, mask_bc, ratio)

                # Update the indices based on new predictions
                z_indices_predict = z_indices_new
                mask_bc = mask_new
...
#TODO3 step1-2: modify the path, MVTM parameters
    parser.add_argument('--load-transformer-ckpt-path', type=str, default='./transformer_checkpoints/square/best_model_weight.pt', help='load ckpt')
    ...
    parser.add_argument('--total-iter', type=int, default=11, help='total step for mask scheduling')
