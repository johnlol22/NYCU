...
class Test_model(VAE_Model):
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        B = img.shape[1]  # Batch size

        # Initial frame is the first and only frame provided
        x_prev = img[0]

        # Generate frames sequentially
        for i in range(0, label.shape[0]):
            # Get current pose label
            p_curr = label[i]

            # Store the pose label for visualization
            label_list.append(p_curr.cpu())

            # Encode previous frame and current label
            f_prev = self.frame_transformation(x_prev)
            p_feat = self.label_transformation(p_curr)

            # Sample from prior for inference - ensure correct shape
